...
void loadCountries(LinkedList<Country> &list, const char *filename);
void requestAction(LinkedList<Country> &list);
void requestSearch(LinkedList<Country> &list);
void requestDeletes(LinkedList<Country> &list);
void insert(LinkedList<Country> &list);
void printLargestPopulation(LinkedList<Country> &list);
string toUpperCase(string input);
void findLetterInCountryCapital(char letter, LinkedList<Country> &list);
Country* findCountryWithLargestPopulation(LinkedList<Country> &list);

int main()
{
  LinkedList<Country> list;
  ...
  do {
    ...
    if (input == "1" || input == "2" || input == "3" || input == "4"
      ...
      if (input == "1") {
        ...
        getline(cin, letter);
        findLetterInCountryCapital(letter.at(0), list);
      }
    }
    else {
...
void requestSearch(LinkedList<Country> &list) {
  ...
  do {
    ...
    temp_country.SetCode(inputUpperCase.c_str());
    country = list.findValue(temp_country);
...
void requestDeletes(LinkedList<Country> &list) {
  ...
  do {
    ...
    Country temp_country;
    temp_country.SetCode(inputUpperCase.c_str());
    errorCode = list.deleteNode(temp_country);
...
void insert(LinkedList<Country> &list) {
  ...
  Country* country = list.findValue(newCountry);
  if (country) {
...
void printLargestPopulation(LinkedList<Country> &list) {
  ...
  mostPopulatedCountry = findCountryWithLargestPopulation(list);
...
//****************************************************
// findLetterInCountryCapital searches for the capital 
// which has the user entered letter and prints the 
// information of that node.
//****************************************************
void findLetterInCountryCapital(char letter, LinkedList<Country> &list) {
  char upperCaseLetter = toupper(letter);
  ListNode<Country> *nodePtr;
  nodePtr = list.GetHead()->GetNextListNode();
  int numCapital = 0;
  while (nodePtr != list.GetHead()) {
    string upperCaseString = toUpperCase(nodePtr->GetValue().GetCapital());
...
Country* findCountryWithLargestPopulation(LinkedList<Country> &list) {
  ...
  ListNode<Country> *nodePtr = list.GetHead()->GetNextListNode();

  int largestPopulation = 0;
  Country* largestPopulationCountry = NULL;
  while (nodePtr != list.GetHead()) {
    if (nodePtr->GetValue().GetPopulation() > largestPopulation) {
      largestPopulation = nodePtr->GetValue().GetPopulation();
      largestPopulationCountry = nodePtr->GetValuePtr();
    }
    nodePtr = nodePtr->GetNextListNode();
  }
...
-------------------------------------------------------------
AU            Australia            Canberra     20,090,437
BR               Brazil            Brasilia    186,112,794
BU             Bulgaria               Sofia      7,262,675
CA               Canada              Ottawa     32,805,041
CN                China             Beijing  1,306,313,812
DO   Dominican Republic       Santo Domingo      8,950,034
...
HU              Hungary            Budapest     10,006,835
IR                 Iran              Tehran     68,017,860
